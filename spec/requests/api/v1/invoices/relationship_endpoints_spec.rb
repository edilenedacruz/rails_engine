...
describe "Invoices relationship endpoint" do
  it "returns a collection of associated transactions" do
    ...
    transactions = Fabricate(:transaction, invoice: invoice)
  ...
  it "returns a collection of associated invoice items" do
    ...
    expect(invoice_invoice_items.all? { |invoice_item_hash| InvoiceItem.find(invoice_item_hash["id"]).invoice_id == invoice.id}).to be true
  end

  it "returns a collection of associated items" do
    invoice = Fabricate(:invoice)
    items = Fabricate.times(5, :item)
    invoice_items = items.map { |item| Fabricate(:invoice_item, item: item, invoice: invoice) }
